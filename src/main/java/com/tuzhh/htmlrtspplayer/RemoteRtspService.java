...
public class RemoteRtspService {
    ...
    private final static String[]    OP_NAME     = {"OPTIONS", "DESCRIBE", "SETUP", "PLAY", "PAUSE", "ANNOUNCE", "RECORD", "GET_PARAMETER", "TEARDOWN",};
                            ...
                            case DATA_RTSP: {
                                ...
                                for (int i = offset; i < len; i++) {
                                    ...
                                    if (expectMsgDataLen == 0) {
                                        if (data[i] == '\n') {
                                            if (buff[buffLen - 1] == '\n' && buff[buffLen - 2] == '\r' && buff[buffLen - 3] == '\n' && buff[buffLen - 4] == '\r') {
                                ...
                                if (opName != null) {
                                    if (sessionInfo.getUrl() == null) {
                                        ...
                                        if (idx > 0) {
                                            ...
                                            if (hsmpUrl != null) {
                                                if (StringUtils.isEmpty(hsmpUrl.get("host"))) hsmpUrl.put("host", "127.0.0.1");
                                    ...
                                    if (sessionInfo.getUrl() != null) {
                                        ...
                                        /*
                                         * if("SETUP".equalsIgnoreCase(opName)) { int div =
                                         * lines.get
                                         * (0).toLowerCase().indexOf("trackID=".toLowerCase());
                                         * if(div > 0) { String trackID = lines.get(0).substring(div
                                         * + "trackID=".length()).split(" ")[0]; } }
                                         */
                                    }
                                    try {
